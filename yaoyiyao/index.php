		...
		if($data['counts']>0){
			...
			///////////用户状态///////////////
				...
				if(!$prize){
					...
					if($count < 3){
						//中奖概率
						if(mt_rand(1,$Probability) == 1){$Random_prize = 'true';}
					}
					//超限
					else{
					...
					//未填表
						...
						if($_POST){
							...
							if(($time-$data['update_time'])>86400){
								$sql = "UPDATE config SET counts=counts-1,day_counts=".$day_counts."-1,update_time=".strtotime(date('Y-m-d',$time))." WHERE project='".$project."'";
							}
							else{
							...
							setcookie('ispost',true,$time+86400);//更新Cookie中POST提交状态
